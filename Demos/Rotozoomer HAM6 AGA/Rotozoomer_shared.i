...
HAM_COPPER_PTR_PLANES                            equ	6          ; bitplane pointer slots emitted into the Copper list
HAM_DISPLAY_BPU                                  equ	6          ; bitplane count programmed into BPLCON0
HAM_AGA_CONTROL_PLANE_BYTES                      equ	1456       ; bytes per fixed HAM-control plane
HAM_AGA_CONTROL_PLANES_BYTES                     equ	2912       ; bytes for both fixed HAM-control planes
HAM_AGA_BPLCON3_RESET                            equ	$0000      ; BPLCON3 palette-bank and LOCT reset
HAM_AGA_BPLCON4_RESET                            equ	$0000      ; BPLCON4 bitplane XOR reset
HAM_AGA_FMODE_RESET                              equ	$0000      ; 16-bit fetch mode
HAM_AGA_BPLCON3_LOCT                             equ	$0200      ; low-order color-table write select
HAM_COPPER_BPLPTR_WORD                           equ	25         ; value slot for initial dynamic row pointers
HAM_COPPER_TEMPORAL_UPPER_BPLPTR_WORD            equ	63         ; value slot for upper temporal row pointers
HAM_COPPER_TEMPORAL_LOWER_BPLPTR_WORD            equ	173        ; value slot for lower temporal row pointers
HAM_COPPER_HALFRATE_BPLPTR_WORD                  equ	283        ; value slot for half-rate row pointers
HAM_COPPER_HALFRATE_BPLPTR_BYTES                 equ	566        ; byte slot for half-rate row pointers
HAM_COPPER_BYTES                                 equ	1412       ; copper list bytes
HAM_CHIP_BLOCK_BYTES                             equ	6856       ; dynamic buffers plus double copper list block bytes
...
HAM_CONTROL_PLANE5_WORD                          equ	$3333      ; fixed HAM control-plane 5 pattern
HAM_CONTROL_PLANE6_WORD                          equ	$6666      ; fixed HAM control-plane 6 pattern
