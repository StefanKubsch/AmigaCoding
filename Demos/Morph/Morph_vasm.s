...
SSAVE_SIZE          equ     40									; saved regs for static path: d2-d5/d7/a2-a6
USAVE_SIZE          equ     28									; saved regs for update path: d5-d7/a2-a5
...
BYTES_PER_ROW       equ     BYTESPERROW							; 1bpl screen row stride
...
ICOSA_EDGE_COUNT    equ     30									; fixed icosahedron edge count
...
_BuildStaticWordMaskFrameAsm::									; public symbol with underscore
    movem.l d2-d5/d7/a2-a6,-(sp)								; save all non-result registers used by this routine
...
.s_done:														; common function exit
    movem.l (sp)+,d2-d5/d7/a2-a6								; restore saved registers
    rts															; return with d0.w = unique touched word count

_UpdateFrameWordsAsm::											; public symbol with underscore
    movem.l d5-d7/a2-a5,-(sp)									; save all caller-visible registers used by this routine
...
.u_old_loop:													; walk all words touched in the previous frame
    ...
    move.w  0(a4,d0.w),d1										; d1 = current frame mask for that word, if any
    move.w  d1,0(a0,d0.w)										; overwrite the plane word directly with the current mask
...
.u_new_loop:													; walk words touched in the current frame but not seen in old pass
    ...
    move.w  0(a4,d0.w),d1										; d1 = current frame mask for that word
    beq.s   .u_new_skip											; skip if the old pass already consumed and cleared this word

    move.w  d1,0(a0,d0.w)										; write the new word directly, plane contents are known zero here
...
.u_done_store:													; finalize the new previous-word metadata
    ...
    movem.l (sp)+,d5-d7/a2-a5									; restore saved registers
...
_DrawBlitterLinesAsm::
    ...
    movea.l a2,a6												; a6 = current edge index pointer
    lea     ICOSA_EDGE_COUNT*2(a6),a0							; a0 = end pointer for the fixed edge list
...
.dl_wait:
    btst.b  #DMAB_BLITTER,(DMACONR-CUSTOMREGS,a1)				; wait while previous line is still running
    bne.s   .dl_wait											; keep setup overlapped with blitter work
    ...
    cmpa.l  a0,a6												; compare current edge pointer with fixed end pointer
    blo     .dl_loop											; draw next edge while entries remain
.dl_final_wait:
    btst.b  #DMAB_BLITTER,(DMACONR-CUSTOMREGS,a1)				; wait until final line finished before buffer is displayed
    bne.s   .dl_final_wait										; keep display handover safe
.dl_done:
    movem.l (sp)+,d2-d7/a2-a6									; restore saved registers
