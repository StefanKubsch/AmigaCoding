...
.wait_init:
	...
	; Write per-frame-constant registers once (saves ~960 writes/frame)
	; BLTCON1 is included here: BSH=15 is guaranteed because we waited above for any
	; previous blit, so no stale BSH value can remain from a prior line-mode blit.
	...
	; Write it once here instead of once per blit (~320 Chip-Bus writes saved/frame).
...
.blit_loop:
	...
	; extract ColumnBits from hi-word before WaitBlit to hide swap latency
...
.blit_done:
	; Disable Blitter Nasty (outstanding blit may still run until next WaitBlit)
...
;   __reg("d1") UWORD        mirrorStart,   e.g. 57
;   __reg("d2") UWORD        totalLines     e.g. 85
; )
;
...
;   d1 = normal loop counter (dbra)
;   d2 = mirror loop counter (dbra)
;   d3 = color word
;   d6 = idx*2 — byte offset into UWORD table (scratch per iteration)
; ---------------------------------------------------------------------------

RAINBOW_MIRROR_SHIFT  equ  1       ; right-shift for half-brightness in mirror region

_UpdateScrollerRainbow::
	movem.l	d2-d3/d6/a2,-(sp)

	; d2 = mirror loop count = totalLines - mirrorStart
	sub.w	d1,d2                   ; d2 = totalLines - mirrorStart

	; Zero-extend phase once: addq.b only modifies bits 0-7, bits 8-15 stay 0.
	and.w	#$FF,d0

	; ---- Normal region: MirrorStart iterations ----
	tst.w	d1
	beq.s	.mirror_loop_start
